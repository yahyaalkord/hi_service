...
class SpinnerLoadingScreen extends StatefulWidget {
  const SpinnerLoadingScreen({Key? key}) : super(key: key);

  @override
  State<SpinnerLoadingScreen> createState() => _SpinnerLoadingScreenState();
}

class _SpinnerLoadingScreenState extends State<SpinnerLoadingScreen>
    with TickerProviderStateMixin {
  late AnimationController firestController;
  late Animation<double> firestAnimation;
  ...
  @override
  void initState() {
    firestController = AnimationController(
      vsync: this,
      duration: Duration(
        seconds: 6,
      ),
    ...
    firestAnimation =
        Tween<double>(begin: -pi, end: pi).animate(firestController)
          ..addListener(() {
            setState(() {});
          })
          ..addStatusListener((status) {
            if (status == AnimationStatus.completed) {
              firestController.repeat();
            } else if (status == AnimationStatus.dismissed) {
              firestController.forward();
            }
          });
...
class MyPainter extends CustomPainter {
  final double firestAngel;
  final double secondAngel;
  final double thirdAngel;
  final double fourthAngel;
  final double fifthAngel;

  MyPainter(this.firestAngel, this.secondAngel, this.thirdAngel,
      this.fourthAngel, this.fifthAngel);

  @override
  void paint(Canvas canvas, Size size) {
    Paint myArc = Paint()
      ..color = Color(0xff0Ba2ff)
